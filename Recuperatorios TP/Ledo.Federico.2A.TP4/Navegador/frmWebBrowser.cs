...
namespace Navegador
{
    public partial class frmWebBrowser : Form
    {
        ...
        private void btnIr_Click(object sender, EventArgs e)
        {
            ...
            direccion = new Uri("http://" + this.txtUrl.Text);
            descargador = new Descargador(direccion);

            descargador.Porcentaje += new Descargador.Porciento(this.ProgresoDescarga);
            descargador.DescargaFinalizada += new Descargador.FinDeLaDescarga(this.FinDescarga);

            this.archivos.guardar("http://" + this.txtUrl.Text);

            Thread miHilo = new Thread(new ThreadStart(descargador.IniciarDescarga));
            miHilo.Start();
        }
